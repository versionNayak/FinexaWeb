...
$(document).ready(function() {
	...
	document.getElementById("idSelectFile").onchange = function(e) {
		//alert(document.getElementById("idSelectFile"));
...
function getSpecificRTAFileNames() {
	...
	serviceurl = "getSpecificFileNameByRTAAndFileCode/" + $('#idRTA').val() + "/" + brokerageFeedConstant;
...
/*$('#idImportButton').change(function(e){
	// Your event handler
	var ddl = document.getElementById("idRTA");
	var selectedValue = ddl.options[ddl.selectedIndex].value;
	if (selectedValue == "1" || selectedValue == "2") {
		
		holdingDrop = $("#idFileType");
		holdingDrop.find('option').remove();
		holdingDrop.append('<option value="">Select File Type</option>');
		holdingDrop.append('<option value="1" name="Excel" >Excel</option>');
		...
		holdingDrop.find('option').remove();
		holdingDrop.append('<option value="">Select File Type</option>');
		holdingDrop.append('<option value="2" name="Text" >Text</option>');
...
$("#idImportButton").on("click", function(event) {
	...
	console.log("Uploading brokerage feed .... ");
	 $("#idProg").show();
	 $("#progressBar").show();
	...
	var form = $("#idFeedUpload")[0];
	if(validateBrokerageMasterForm(form, fileName)){
		...
		uploadAjax = $.ajax({
		        ...
		        url: ClientServiceUrl+'/uploadBrokerageFeed',
		        data: data,
		        ...
		        success: function (data) {
		        	...
		        	if(data.status) {
		        		...
		        		$("#alertMsg").hide();
		        		 bootbox.confirm({
		        			 title: "Error Message",
		        			 message: "Failed to upload Brokerage Feed file!",
		        			 buttons: {
			        	            cancel: {
