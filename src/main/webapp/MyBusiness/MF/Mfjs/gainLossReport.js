...
$(document).ready(function() {
    ...
    btn.onclick = function() {
    	...
    	var validate;
    	validate = validateGainLossReportGeneration($('#idGainLossReportForm'));
    	
		if(validate == true && errTransReport == 0) {
    		var formData = $('#idGainLossReportForm').serializeToJSON();
			...
			$.ajax({
				async: false,
				url: ClientServiceUrl + "generatePortfolioGainLossReport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&fromDate=" + fromDate + 
	...
	$("#exportGainLossReport").on("click", function(event) {
		...
		var validate;
		validate = validateGainLossReportExport($('#idGainLossReportForm'));
		...
		if(validate == true && errTransReport == 0) {
			...
			$.ajax({
				async: false,
				url: ClientServiceUrl + "portfolioGainLossExport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat + "&fromDate=" + fromDate + 
						"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + SchemeName + "&advisorID=" + loggedInUser.id,
				method: "GET",
				beforeSend: function (xhr){ 
					xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
				...
				success: function (data) {
					...
					if(data == "No Data") {
						...
						xhr.open( "GET", serviceIP + "/clientservice/" + 'downloadHandlerForReport/gainLoss/' + reportFormat, true);
						xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
