...
function getSpecificRTAFileNames() {
	...
	serviceurl = "getSpecificFileNameByRTAAndFileCode/" + $('#idRTA').val() + "/" + dailyFeedConstant;
...
$("#idImport").on("click", function(event) {
	...
	console.log("Uploading transaction feed .... ");
	 $("#idProg").show();
	 $("#progressBar").show();
	...
	var form = $("#idDailyFeedsForm")[0];
	if(validateDailyFeedForm(form, fileName)){
		...
		uploadAjax = $.ajax({
		        ...
		        url: ClientServiceUrl+'backOffice/uploadTransactionMaster',
		        data: data,
		        ...
		        success: function (data) {
		        	...
		        	if(data.status) {
		        		...
		        		if(data.primaryKeyNotFound) {
		        		 ...
		        		 bootbox.confirm({
		        			 title: "Error Message",
		        			 message: "Failed to upload Transaction Feed file!",
		        			 buttons: {
			        	            cancel: {
			        	                label: '<i class="fa fa-times"></i> Cancel'
			        	            },
