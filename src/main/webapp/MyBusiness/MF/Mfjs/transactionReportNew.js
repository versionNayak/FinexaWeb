...
$(document).ready(function(){
	...
	$('#wait').hide();
		    ...
		    /*$("#idFHouseDrop").mouseout(function(){
		        alert("idFHouseDrop mouseout function()");
		    ...
		    btn.onclick = function() {
				...
				//alert("fundHouseName "+fundHouseName);
				/*if(fundHouseName != null){
					if(isin == null){
			    ...
			    validate = validateTransactionReportGeneration($('#idTransactionReportForm'),clientId,fundHouseName,isin,checkedFamilyMemberId);
			    ...
			    if(errTransReport == 0 && validate == true) {
					...
					//console.log("client ID = " + clientId + "fundHouseName = " + fundHouseName + "isin = " + isin)
					
					$.ajax({
						async: false,
						url: ClientServiceUrl + "generateTransactionReportNew?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId+ "&fromDate=" + fromDate + 
						...
						success: function (data) {
							...
							if(data == "No Data") {
								...
								window.open("resources/TransactionReportNew.html", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=400,height=400");
							}
						},
			...
			$("#exportReport").on("click", function(event) {
				...
				validate = validateTransactionReportExport($('#idTransactionReportForm'),clientId,fundHouseName,isin,reportFormat,checkedFamilyMemberId);
				...
				if(validate == true && errTransReport == 0) {
					...
					$.ajax({
						async: false,
						url: ClientServiceUrl + "exportTransactionReportNew?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat + "&fromDate=" + fromDate + 
								"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + isin + "&advisorID=" + loggedInUser.id,
						method: "GET",
						beforeSend: function (xhr){ 
							xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
						...
						success: function (data) {
							...
							if(data == "No Data") {
								...
								if(reportFormat == "pdf") {
									var fileName = "TransactionReportNew.pdf";
								} else if(reportFormat == "excel") {
									var fileName = "TransactionReportNew.xls";
								}
								
								var xhr = new XMLHttpRequest();
								xhr.open( "GET", serviceIP + "/clientservice/" + 'downloadHandlerForReport/transactionNew/' + reportFormat, true);
								xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
