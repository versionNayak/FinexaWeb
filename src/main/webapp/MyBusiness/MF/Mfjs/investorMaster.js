...
function getSpecificRTAFileNames() {
	...
	serviceurl = "getSpecificFileNameByRTAAndFileCode/" + $('#idRTA').val() + "/" + investorMasterConstant;
...
$("#idImportButton").on("click", function(event) {
	...
	console.log("Uploading investor feed .... ");
	 $("#idProg").show();
	 $("#progressBar").show();
	...
	if(validateInvestorMasterForm(form, fileName)){
		...
		window.setTimeout(function() {
			...
			data.append("nameFileType", form.nameFileType);
		uploadAjax = $.ajax({
				type: "POST",
		        enctype: 'multipart/form-data',
		        url: ClientServiceUrl+'/uploadInvestorFeed',
		        data: data,
		        ...
		        success: function (data) {
		        	...
		        	if(data.status) {
		        		...
		        		if(data.primaryKeyNotFound) {
		        		 ...
		        		 bootbox.confirm({
		        			 title: "Error Message",
		        			 message: "Failed to upload Investor Feed file!",
		        			 buttons: {
			        	            cancel: {
			        	                label: '<i class="fa fa-times"></i> Cancel'
			        	            },
