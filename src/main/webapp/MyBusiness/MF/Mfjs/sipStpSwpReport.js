...
/*	$("#idClients").change(function() {
	...
	$('input[type="radio"][name="reportFormat"]').change(function() {
		...
		console.log("report Format is : " + reportFormat);
	});
    ...
    btnId.onclick = function() {
	    ...
	    var validate = validateSipStpSwpGeneration($("#idSipStpSwpReportForm"),clientId,fundHouseName,isin,reportTypeArr.join(""),checkedFamilyMemberId);
    	
    	if(validate) {
    		var formData = $('#idSipStpSwpReportForm').serializeToJSON();
			...
			*/
		    ...
		    console.log("Report Types are: " + reportTypeArr.join(" "));
			
			$.ajax({
				async: false,
//				url: ClientServiceUrl + "generateSipStpSwpReport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&fromDate=" + fromDate + 
...
//						"&reportType=" + reportTypeArr.join(" ") + "&advisorID=" + loggedInUser.id,
				...
				success: function (data) {
					...
					if(data == "No Data") {
						...
						window.open("resources/SipStpSwpReport.html", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=400,height=400");	
					}
				},
	...
	$("#exportSipStpSwpReport").click(function() {
		...
		//alert("reportFormat "+reportFormat);
		var validate = validateSipStpSwpReportExport($("#idSipStpSwpReportForm"),clientId,fundHouseName,isin,reportTypeArr.join(""),reportFormat,checkedFamilyMemberId);
		...
		if(validate) {
			...
			*/
			...
			$.ajax({
				async: false,
				/*url: ClientServiceUrl + "sipStpSwpExport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat +"&fromDate=" + fromDate + 
						"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + SchemeName + 
						"&reportType=" + reportTypeArr.join(" ") + "&advisorID=" + loggedInUser.id,*/
				url: ClientServiceUrl + "sipStpSwpExport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat +"&fromDate=" + fromDate + 
						"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + isin + 
				...
				success: function (data) {
					...
					if(data == "No Data") {
						...
						if(reportFormat == "pdf") {
							var fileName = "SipStpSwpReport.pdf";
						} else if(reportFormat == "excel") {
							var fileName = "SipStpSwpReport.xls";
						}
						
						var xhr = new XMLHttpRequest();
						xhr.open( "GET", serviceIP + "/clientservice/" + 'downloadHandlerForReport/' + reportTypeArr.join(" ") + "/" + reportFormat, true);
						xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
