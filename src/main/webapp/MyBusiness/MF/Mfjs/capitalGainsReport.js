...
$(document).ready(function() {
    ...
    btn.onclick = function() {
    	...
    	var validate;
    	//validate = validateGainLossReportGeneration($('#idCapitalGainsReportForm'));
    	validate = validateGainLossReportGeneration($('#idCapitalGainsReportForm'),clientId,fundHouseName,isin,checkedFamilyMemberId);
    	
		if(validate == true && errTransReport == 0) {
    		var formData = $('#idCapitalGainsReportForm').serializeToJSON();
			...
			$.ajax({
				async: false,
//				url: ClientServiceUrl + "generateCapitalGainsReport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&fromDate=" + fromDate + 
//						"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + SchemeName + "&advisorID=" + loggedInUser.id,
				...
				success: function (data) {
					...
					if(data == "No Data") {
						...
						window.open("resources/CapitalGainsReport.html", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=400,height=400");
					}
				},
	...
	$("#exportGainLossReport").on("click", function(event) {
		...
		var validate;
		//validate = validateGainLossReportExport($('#idCapitalGainsReportForm'));
		validate = validateGainLossReportExport($('#idCapitalGainsReportForm'),clientId,fundHouseName,isin,reportFormat,checkedFamilyMemberId);
...
//			console.log(SchemeName);
			...
			$.ajax({
				async: false,
//				url: ClientServiceUrl + "exportCapitalGainsReport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat + "&fromDate=" + fromDate + 
//						"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + SchemeName + "&advisorID=" + loggedInUser.id,
				url: ClientServiceUrl + "exportCapitalGainsReport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat + "&fromDate=" + fromDate + 
				"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + isin + "&advisorID=" + loggedInUser.id,
				method: "GET",
				...
				success: function (data) {
					...
					if(data == "No Data") {
						...
						if(reportFormat == "pdf") {
							var fileName = "CapitalGainsReport.pdf";
						} else if(reportFormat == "excel") {
							var fileName = "CapitalGainsReport.xls";
						}
						
						var xhr = new XMLHttpRequest();
						xhr.open( "GET", serviceIP + "/clientservice/" + 'downloadHandlerForReport/capitalGains/' + reportFormat, true);
						xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
