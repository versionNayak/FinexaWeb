function validatePortfolioValuationReportGeneration(form,clientId,fundHouseName,isin,checkedFamilyMemberId) {
	...
	if (!hasValue(lAsOnDate.value)) {
		...
		var chkDate=-1;
		chkDate = isPastDate(lAsOnDate.value);
		if (!chkDate){
			document.getElementById('alertAsOnDate').innerHTML="Date cannot be greater than current date";
			lAsOnDate.style.border = "2px solid red";
			errPGLReport=1;
		}
	}
...
function validatePortfolioValuationReportExport(form,clientId,fundHouseName,isin,reportFormat,checkedFamilyMemberId) {
