...
$(document).ready(function(){
		...
		console.log("advisorID: " + loggedInUser.id);
	    ...
	    btnId.onclick = function() {
	    	...
	    	var validate;
	    	validate = validateDividendReportGeneration($('#idDividendReportForm'),clientId,fundHouseName,isin,checkedFamilyMemberId);
	    	
			if(validate == true && errTransReport == 0) {
	    		var formData = $('#idDividendReportForm').serializeToJSON();
				...
				$.ajax({
					async: false,
//					url: ClientServiceUrl + "generateDividendReportNew?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&fromDate=" + fromDate + 
//							"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + SchemeName + "&advisorID=" + loggedInUser.id,
					...
					success: function (data) {
						...
						if(data == "No Data") {
							...
							window.open("resources/DividendReportNew.html", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=400,height=400");
						}
					},
		...
		$("#exportDividendReport").on("click", function(event) {
			
			var checkedFamilyMemberId=[];
		    $('.fmCheck:checked').each(function(){   
			   checkedFamilyMemberId.push(this.id);
		    });
			//alert("checkedFamilyMemberId " + checkedFamilyMemberId);
		    if (clientId == null || clientId == "") {
				document.getElementById('alertClients').innerHTML="Please select a Client";
				lClients.style.border = "2px solid red";
				errTransReport = 1;
			...
			var validate;
			validate = validateDividendReportExport($('#idDividendReportForm'),clientId,fundHouseName,isin,reportFormat,checkedFamilyMemberId);
...
//				console.log(SchemeName);
				...
				$.ajax({
					async: false,
//					url: ClientServiceUrl + "dividendReportExportNew?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat + "&fromDate=" + fromDate + 
//							"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + SchemeName + "&advisorID=" + loggedInUser.id,
					url: ClientServiceUrl + "dividendReportExportNew?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat + "&fromDate=" + fromDate + 
					"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + isin + "&advisorID=" + loggedInUser.id,
					method: "GET",
					...
					success: function (data) {
						...
						if(data == "No Data") {
							...
							if(reportFormat == "pdf") {
								var fileName = "DividendReportNew.pdf";
							} else if(reportFormat == "excel") {
								var fileName = "DividendReportNew.xls";
							}
							
							var xhr = new XMLHttpRequest();
							xhr.open( "GET", serviceIP + "/clientservice/" + 'downloadHandlerForReport/dividendNew/' + reportFormat, true);
							xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
