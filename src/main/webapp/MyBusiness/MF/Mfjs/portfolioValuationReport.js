...
$(document).ready(function() {
    ...
    var btn = document.getElementById("showPortfolioValuationReport");
    ...
    btn.onclick = function() {
	   ...
	   /* if (clientId == null || clientId == "") {
    	...
    	var validate;
    	//validate = validatePortfolioValuationReportGeneration($('#idPortfolioValuationReportForm'));
    	validate = validatePortfolioValuationReportGeneration($('#idPortfolioValuationReportForm'),clientId,fundHouseName,isin,checkedFamilyMemberId);
    	if(validate == true && errTransReport == 0) {
    		var formData = $('#idPortfolioValuationReportForm').serializeToJSON();
...
//			console.log(SchemeName);
			$.ajax({
				async: false,
//				url: ClientServiceUrl + "generatePortfolioValuationReport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&asOnDate=" + asOnDate + 
//						"&fundHouse=" + fundHouseName + "&schemeName=" + SchemeName + "&advisorID=" + loggedInUser.id,
				...
				success: function (data) {
					...
					if(data == "No Data") {
						...
						window.open("resources/PortfolioValuationReport.html", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=400,height=400");
					}
				},
	...
	$("#exportPortfolioValuationReport").on("click", function(event) {
	   ...
	   /* if (clientId == null || clientId == "") {
		...
		var validate;
		validate = validatePortfolioValuationReportExport($('#idPortfolioValuationReportForm'),clientId,fundHouseName,isin,reportFormat,checkedFamilyMemberId);
		...
		if(validate == true && errTransReport == 0) {
			...
			$.ajax({
				async: false,
//				url: ClientServiceUrl + "portfolioValuationExport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat + "&asOnDate=" + asOnDate + 
//						"&fundHouse=" + fundHouseName + "&schemeName=" + SchemeName + "&advisorID=" + loggedInUser.id,
				url: ClientServiceUrl + "portfolioValuationExport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat + "&asOnDate=" + asOnDate + 
				"&fundHouse=" + fundHouseName + "&schemeName=" + isin + "&advisorID=" + loggedInUser.id,
				method: "GET",
				...
				success: function (data) {
					...
					if(data == "No Data") {
						...
						if(reportFormat == "pdf") {
							var fileName = "PortfolioValuationReport.pdf";
						} else if(reportFormat == "excel") {
							var fileName = "PortfolioValuationReport.xls";
						}
						
						var xhr = new XMLHttpRequest();
						xhr.open( "GET", serviceIP + "/clientservice/" + 'downloadHandlerForReport/portfolioValuation/' + reportFormat, true);
						xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
