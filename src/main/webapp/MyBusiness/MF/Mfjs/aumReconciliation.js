...
function getSpecificRTAFileNames() {
	...
	serviceurl = "getSpecificFileNameByRTAAndFileCode/" + $('#idRTA').val() + "/" + aumReconciliationConstant;
...
$("#idImport").click(function(event){
	
	console.log("Uploading aum feed.... ");
	
	$("#idProg").show();
	$("#progressBar").show();
	$("#progressBarCanvas").show();
	
	var form = $("#idAumReconciliationForm")[0];
	if(validateAUMReconciliationUploadForm(form, fileName)){
		//alert(fileName);
		...
		window.setTimeout(function() {
			...
			uploadAjax = $.ajax({
		        ...
		        url: ClientServiceUrl+'backOffice/uploadAumFeed',
		        data: data,
		        ...
		        success: function (data) {
		        	...
		        	if(data.status) {
		        		...
		        		if(data.primaryKeyNotFound) {
		        		 ...
		        		 bootbox.confirm({
		        			 title: "Error Message",
		        			 message: "Failed to upload AUM Feed file!",
		        			 buttons: {
			        	            cancel: {
			        	                label: '<i class="fa fa-times"></i> Cancel'
			        	            },
