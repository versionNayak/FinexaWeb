...
$(document).ready(function(){
	    ...
	    btnId.onclick = function() {
	    	...
	    	var validate;
	    	validate = validateDividendReportGeneration($('#idDividendReportForm'));
			...
			if(validate == true && errTransReport == 0) {
				...
				$.ajax({
					async: false,
					url: ClientServiceUrl + "generateDividendReport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&fromDate=" + fromDate + 
					...
					success: function (data) {
						...
						if(data == "No Data") {
							...
							window.open("resources/DividendReport.html", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=400,height=400");
						}
					},
		...
		$("#exportDividendReport").on("click", function(event) {
			...
			var validate;
			validate = validateDividendReportExport($('#idDividendReportForm'));
			...
			if(validate == true && errTransReport == 0) {
				...
				$.ajax({
					async: false,
					url: ClientServiceUrl + "dividendReportExport?clientId=" + clientId + "&familyMemberIdArr=" + checkedFamilyMemberId + "&reportFormat=" + reportFormat + "&fromDate=" + fromDate + 
							"&toDate=" + toDate + "&fundHouse=" + fundHouseName + "&schemeName=" + SchemeName + "&advisorID=" + loggedInUser.id,
					method: "GET",
					beforeSend: function (xhr){ 
						xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
					...
					success: function (data) {
						...
						if(data == "No Data") {
							...
							if(reportFormat == "pdf") {
								var fileName = "DividendReport.pdf";
							} else if(reportFormat == "excel") {
								var fileName = "DividendReport.xls";
							}
							
							var xhr = new XMLHttpRequest();
							xhr.open( "GET", serviceIP + "/clientservice/" + 'downloadHandlerForReport/dividend/' + reportFormat, true);
							xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
