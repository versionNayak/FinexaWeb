			...
			error: function (jqXHR, exception) {
		       ...
		       // alert("Error Msg : \n" + ClientServiceUrl + path +"\n "+ msg);
		        ...
		        if (JSON.parse(jqXHR.responseText).errorMessage == undefined) {
		         ...
		         $("#idClient").load("resources/errorPage.html");
		         if (jqXHR.status == 403) {
		        	 $("#idBusiness").load("../authorisationErrorPage.html");
			     } else {
			         $("#idBusiness").load("resources/errorPage.html");
			     }
		         $("#idBuinessMasters").load("resources/errorPage.html");
...
function populateRelationDrop(dropId) {
	...
	function onRelationSuccess(data) {
		...
		//dropId.find('<option value="0">self</option>').remove();
...
function editPageClient(path,heading)
{
				sessionStorage.setItem("PAGE_MODE", "EDIT");
				sessionStorage.setItem("FAMILY_ADD_CLIENT", "NO");
				sessionStorage.setItem("LifeExpecTancy_ADD_CLIENT", "NO");
...
{				
				...
				//$("#dashbord").empty();//don't delete
				//$("#dashbord").load(path);//don't delete
...
function editPageBackOffice(path,heading)
{
				sessionStorage.setItem("PAGE_MODE", "EDIT");
                $("#idBackOffice").empty();
                $(".dashboardheading").html("");
				$("#idBackOffice").load(path);
                $("#page-content-wrapper").css("height","auto");
				$(".form-section-container").addClass("height1257px");
				$(".nonload").css("display","block");
				$("#top-nav-bar").show();
...
function addPageBackOffice(path,heading) {
...
function addPageExceptionHandling(path,heading) {
	
	sessionStorage.setItem("PAGE_MODE", "ADD");
    
    $("#idExceptionHandling").empty();
	$("#idExceptionHandling").load(path);
...
function editPageExceptionHandling(path,heading)
{
				sessionStorage.setItem("PAGE_MODE", "EDIT");
                $("#idExceptionHandling").empty();
                $(".dashboardheading").html("");
...
function editPageBusiness(path,heading)
{
     ...
     function editPageBusinessRiskProfile(path,heading)
     {
	alert("editPageBusiness");
	sessionStorage.setItem("PAGE_MODE", "EDIT");
	$("#idBusiness").empty();
	$("#idBusiness").load(path);
...
function getAdvisorClientData(method, jsondata, path, callbackSuccess) {
...
function getClientDataBeforeLogin(method, jsondata, path, callbackSuccess) {
