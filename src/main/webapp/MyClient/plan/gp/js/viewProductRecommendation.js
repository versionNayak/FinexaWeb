...
jQuery(document).ready(function ($) {
	...
	function downloadSaveDate(serialNo) {
	...
	function loadProductReco(saveDate) {
		...
		var lumpsumDisplayFlag = 0; // if 0 then hide the column, if 1 then show the column
		var sipDisplayFlag = 0; // if 0 then hide the column, if 1 then show the column
		
		var date = saveDate.replace(/\//g, "");
		var selectedClientId = sessionStorage.getItem("SELECTED_CLIENT_ID");
		var loggedUser = JSON.parse(sessionStorage.getItem("LOGGED_IN_USER"));
		var serviceurlForLoadingSavedReco = "getLastSavedProductReco/"+advisorUserId+"/" + selectedClientId + "/" + goalId + "/GP/"+date;
...
//					alert(tableString);
					...
					for (var prodIndex = 0; prodIndex < productLength; prodIndex ++) {
						if (value.productValue[prodIndex] > 0) {
							...
							tableString = tableString + '<td id="lump" style="vertical-align:top;width:230px;" >' + maskAmountValue(Math.round(parseFloat(value.lumpsumAmt[prodIndex]))) +'</td>';
							tableString = tableString + '<td id="sip" style="vertical-align:top;width:230px;">' + maskAmountValue(Math.round(parseFloat(value.sipAmount[prodIndex]))) +'</td>';
...
//				$("#idProductTableModal").append(tableString);
				/*if (lumpsumDisplayFlag == 0) {
					alert(lumpsumDisplayFlag);
					$.each(data,function(index, value) {
						var idTablleText = "idTableText" + index;
						$('#'+idTablleText+' td:nth-child(3),#'+idTablleText+' th:nth-child(3)').hide().addClass('noprint');
					});
					$('#idLumpHeader').hide();
				} else {
					$('#idLumpHeader').show();
				...
				if (sipDisplayFlag == 0) {
					alert(sipDisplayFlag);
					$('#idSavedProductTable td:nth-child(6),#idSavedProductTable th:nth-child(6)').hide().addClass('noprint');
					$('#idSavedProductTable td:nth-child(7),#idSavedProductTable th:nth-child(7)').hide().addClass('noprint');
				}*/
			}
...
//		alert("SaveDate" + date);
	...
	$.ajax({
		...
		'&mode='+selectedMode+'&lumpsumValue='+lumpsumForSIP+'&glideNonglideMode='+vglideNonglideMode+'&advisorId='+advisorMasterId,
		async: false,
		dataType: 'json',
		beforeSend: function (xhr){ 
			xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
		...
		success: function (data) {
			$("#idSavedProductTableGenerate").empty();
			tableString = "";
			JsonData = data;
...
//					alert(tableString);
					...
					$.each(value.productName,function(index2, value2) {
						var siptenure;
						if(value.sipTenure != 0){
							siptenure  = value.sipTenure;
						}else{
							siptenure = "N/A";
						}
						
						var lumpsumAmount = maskAmountValue(Math.round(parseFloat(value.lumpsumAmt/value.productName.length).toFixed(2)));
						//alert("lumpsumAmount = " + lumpsumAmount);
						var sipAmt = maskAmountValue(Math.round(parseFloat(value.sipAmt/value.productName.length).toFixed(2)));
						//alert("sipAmt = " + sipAmt);
						
						tableString = tableString + '<tr class="amtenur">' + 
						'<td id="lump" style="vertical-align:top">' + lumpsumAmt +'</td>'+
						'<td id="sip" style="vertical-align:top">' + sipAmt +'</td>'+
						'<td style="padding-right: 16px">' + siptenure +'</td></tr>';
...
//				$("#idProductTableModal").append(tableString);
	...
	// load all the existing plans of advisor
	 $.ajax({
		 type: 'GET',
		 url: serviceIP + "/clientservice/" + 'getAdvisorProductReco?advisorId='+advisorUserId+'&clientId='+vClientId+'&goalId='+goalId+'&module=GP',
		 async: true,
		 dataType: 'json',
		 beforeSend: function (xhr){ 
				xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
...
//					When the user clicks the button, open the modal 
					btn.onclick = function() {
						...
						var selectedLength = selectedButtonId.length;
						var selectedId = selectedButtonId.substring((selectedIndex + 1),selectedLength);
						var recoSaveDateView = $("#idRecoSaveDate"+selectedId).text();
						loadProductReco(recoSaveDateView);
...
//		alert("Hi");
		...
		$.ajax({
			type: 'GET',
			url: serviceIP + "/clientservice/" + 'saveAdvisorProductReco?advisorId='+advisorUserId+'&clientId='+vClientId+'&goalId='+goalId+'&module=GP&jsonData='+jsonString,
			async: true,
			beforeSend: function (xhr){ 
				xhr.setRequestHeader('Authorization', "Bearer "+sessionStorage.getItem("sessionID"));
		    },
