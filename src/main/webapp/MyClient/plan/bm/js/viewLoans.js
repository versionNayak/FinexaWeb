...
$("#idDivLoan3").hide();
$("#idDivLoan2").hide();
$("#idIndividualLoan").hide();
jQuery(document).ready(function ($) {

	
	$("#IndividualLoanstbl").load("bm/viewIndividualLoanstbl.html");
	$("#loanstotaltbl").load("bm/viewLoanstotaltbl.html");
...
//		alert("unloaded");
	...
	$("#idDownload").click(function(){
		...
		if (errorFlag == 0) {
			...
			//xhr.open( "GET", REQUEST_URL_BM+'/downloadLoanReport?clientId='+vClientId+'&mode='+mode, true);
		...
		success: function (data) {
			...
			$.each(data,
				...
				if(value.originalPrincipal != 0) {
					originalPrincipal = maskAmountValue(Math.round(value.originalPrincipal));
				}
				var emi = "N/A";
				if(value.emi != 0) {
					emi = maskAmountValue(Math.round(value.emi));
				}
				
				var outstandingPrincipal = "N/A";
				if(value.outstandingPrincipal != 0) {
					outstandingPrincipal = maskAmountValue(Math.round(value.outstandingPrincipal));
				}
				
				$('#idLoadCurrentTable').append(
...
//					$('#idLoanCurrentTable td:nth-child(4),#idLoanCurrentTable th:nth-child(4)').hide();
                    ...
                    series: {
                        events: {
                            legendItemClick: function (e) {
                             //   this.legendItem.visible=false;

                                if(indexItem.length===totallength) {
                                    return false;
                                }
                                if(indexItem.length<(totallength-1))
                                {
                                    indexItem.push(e.target.index);
                                }else {
                                	if((indexItem.indexOf(e.target.index))!==-1) {
										var index = indexItem.indexOf(e.target.index);
										if (index !== -1) indexItem.splice(index, 1);
										return true;
                                	} else {
                                    	return false;
                                	}
	...
	var innerHtmlString = '<div data-u="image"><div id="idMaxIndiLoans" class="popup" style="position: relative; top: 30px; left: 64px">'+
					'<img src="../Common/assets/images/maximg.png" class="maxbtn"/></div>';
	$.ajax({
		type: 'GET',
		async:false,
		...
		success: function (data) {
			$.each(data,
				...
				'<div><div><div class="container-fluid"><table width="97%" class="individualoan">'+
							'<thead>'+
								'<tr>'+
									'<th>Year</th>'+
				...
				$.each(value.clientFamilyLoanOutputList,
					...
					var emiAmt = maskAmountValue(Math.round(value1.emiAmount));
			...
			document.getElementById("idIndividualLoanModal").innerHTML=htmlContentModal;
			$("#idDivLoan1").show();
			$("#idDivLoan3").show();
			$("#idDivLoan2").show();
			$("#idIndividualLoan").show();
//			document.getElementById("idIndividualLoan").innerHTML = innerHtmlString;
		},
		error: function (jqXHR) {
			if(jqXHR.status == 401){
...
//	Get the button that opens the modal
	var btn = document.getElementById("idMaxTotalLoan");
