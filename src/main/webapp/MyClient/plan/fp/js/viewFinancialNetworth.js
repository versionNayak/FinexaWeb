$(document).ready(function(){
	...
	$(".Expensesright").click(function(){
		...
		{	       
			...
			//$("#idfpHeading").html("Financial Ratios");
			$(".glidnonglid").hide();
...
//	When the user clicks anywhere outside of the modal, close it
			...
			$.each(data, function (mapKey, mapValue) {
				if(mapKey=='rootMap'){
						...
						if(assetindex=='Assets'){		
							$.each(assets, function (typesubassetsindex, typesubassets) {
								...
								if(typesubassetsindex=='Personal Assets'){
									...
									//console.log("totalAssetMap['Personal Assets'] "+totalAssetMap["Personal Assets"]);
									if(totalAssetMap["Personal Assets"]!=0){//piyali
									...
									$.each(typesubassets, function (typesubassetsProductsindex, typesubassetsProducts) { 			
										$.each(typesubassetsProducts, function (productindex, productvalue) {
											...
											assetPieChartObject['y']= parseFloat((totalProductType/totalAssetValue)*100);
											assetPieChartObject['color']=assetcolorNumbering[assetcolorCount];
											//console.log(JSON.stringify(assetPieChartObject));
											...
											//alert("totalProductType: " + totalProductType);
										});
									});
								  }	//piyali
								}
								...
								if(typesubassetsindex.includes("Investment"))
								{
									$("#assetClasses").append("<tr class='bluebg'><td class='padding5px'><b> Total "+typesubassetsindex+"</b></td>"+
											...
											"<td>"+(totalAssetMap["Investment Assets"]==null || totalAssetMap["Investment Assets"]== ""?0:((totalAssetMap["Investment Assets"]/totalAssetValue)*100).toFixed(2))+"%</td>"+
									"</tr> ");
									$("#popupAssetClasses").append("<tr class='bluebg'><td class='padding5px'><b> Total "+typesubassetsindex+"</b></td>"+
											"<td><b>"+((totalassetType > 0 )? parseFloat(totalassetType).toFixed(2) : 0.00)+"</b></td>"+
											"<td>"+(totalAssetMap["Investment Assets"]==null || totalAssetMap["Investment Assets"]== "" ?0:((totalAssetMap["Investment Assets"]/totalAssetValue)*100).toFixed(2))+"%</td>"+
									"</tr> ");}		

								if(typesubassetsindex.includes("Personal"))
								{
									...
									$("#assetClasses").append("<tr class='bluebg'><td class='padding5px'><b> Total "+typesubassetsindex+"</b></td>"+
											"<td><b>"+((totalassetType > 0)? maskAmountValue(Math.round(totalassetType)) : 0.00)+"</b></td>"+
											"<td>"+(totalAssetMap["Personal Assets"]==null || totalAssetMap["Personal Assets"]== "" ?0:((totalAssetMap["Personal Assets"]/totalAssetValue)*100).toFixed(2))+"%</td>"+
									"</tr> ");
									$("#popupAssetClasses").append("<tr class='bluebg'><td class='padding5px'><b> Total "+typesubassetsindex+"</b></td>"+
											"<td><b>"+((totalassetType > 0 )? parseFloat(totalassetType).toFixed(2) : 0.00)+"</b></td>"+
											"<td>"+(totalAssetMap["Personal Assets"]==null || totalAssetMap["Personal Assets"]== "" ?0:((totalAssetMap["Personal Assets"]/totalAssetValue)*100).toFixed(2))+"%</td>"+
					...
					$("#popupAssetClasses").append("<tr class='grybg'><td><b> Total Asset : (A) + (B)</b></td>"+
							"<td><b>"+ ((totalAssetMap["Personal Assets"] > 0 && totalAssetMap["Investment Assets"] > 0)? parseFloat(totalAssetMap["Personal Assets"]+totalAssetMap["Investment Assets"]).toFixed(2) : 0.00)+"</b></td>"+
							"<td>100%</td>"+"</tr> ");
				...
				$.each(mapValue, function (productsindex, productValue) { 
					if(productsindex.includes("Loan")){
						...
						liabilityPieChartObject['y']= parseFloat((productValue/(totalAssetMap["Liabilities"]))*100);
	...
	function populateDetailsNetworth(data)
	{
		...
		$.each(data, function (mapKey, mapValue) { 
			if(mapKey=='totaltypeValueMap'){
				totalAssetMap["Personal Assets"]=(mapValue["Personal"] > 0)? mapValue["Personal"] : 0;   //piyali
				totalAssetMap["Investment Assets"] = (mapValue["Investment"] > 0)? mapValue["Investment"] : 0;  //piyali
				totalAssetMap["Liabilities"] = (mapValue["Liabilities"] > 0)? mapValue["Liabilities"] : 0;  //piyali
					...
					if(assetindex=='Assets'){
						...
						$.each(assets, function (typesubassetsindex, typesubassets) { 
							...
							if(typesubassetsindex == 'Investment Assets'){
								...
								$.each(typesubassets, function (typesubassetsProductsindex, typesubassetsProducts) { 
									...
									$.each(typesubassetsProducts, function (productindex, productvalue) { 
										...
										$.each(productvalue.networthDetails, function (productNameindex, productNamevalue) {
                                            if(productNamevalue.currentValue > 0){
								...
								if (isNaN(totalPersonalAssetPercentage)) {
									totalPersonalAssetPercentage = 0;
								}
								$("#networthDetails").append("<tr class='bluebg'><td class='padding5px'><b> Total "+typesubassetsindex+"</b></td>"+
						...
						$("#networthDetails").append("<tr class='grybg'><td><b> Total Asset : (A) + (B)</b></td>"+
								...
								"<td>"+((total/totalAssetValue)*100).toFixed(2)+"%</td>"+"</tr> ");

						$("#networthDetailsPopup").append("<tr class='grybg'><td><b> Total Asset : (A) + (B)</b></td>"+
								"<td><b>"+maskAmountValue(Math.round(totalAssetValue))+"</b></td>"+
								"<td>"+((total/totalAssetValue)*100).toFixed(2)+"%</td>"+"</tr> ");
					}else{
						//if(totalAssetMap["Liabilities"]!=0){ // piyali
						$.each(assets, function (typesubassetsindex, typesubassets) {
							...
							$.each(typesubassets, function (typesubassetsProductsindex, typesubassetsProducts) { 	
								...
								$.each(typesubassetsProducts, function (productindex, productvalue) { 
									...
									$("#networthDetails").append("<tr><td>"+productvalue.productName+"</td>"+
											"<td>"+((productvalue.currentValue > 0)? maskAmountValue(Math.round(productvalue.currentValue)) : 0.00)+"</td><td>"+((parseFloat(productvalue.currentValue).toFixed(2)/totalAssetMap["Liabilities"])*100).toFixed(2)+"%</td></tr> ");
									$("#networthDetailsPopup").append("<tr><td>"+productvalue.productName+"</td>"+
											"<td>"+((productvalue.currentValue > 0)? maskAmountValue(Math.round(productvalue.currentValue)) : 0.00)+"</td><td>"+((parseFloat(productvalue.currentValue)/totalAssetMap["Liabilities"])*100).toFixed(2)+"%</td></tr> ");
									/*$("#libilitiesPopup").append("<tr><td>"+productvalue.productName+"</td>"+
											"<td>"+((productvalue.currentValue > 0)? parseFloat(productvalue.currentValue).toFixed(2)/totallibility : 0.00)+"</td><td>"+(parseFloat(productvalue.currentValue).toFixed(2)/totallibility)*100+"</td></tr> ");*/

								});
							});

							$("#networthDetails").append("<tr class='grybg'><td><b> Total Liabilities</b></td>"+
									"<td><b>"+maskAmountValue(Math.round(totallibility))+"</b></td>"+
									"<td>"+(totalAssetMap["Liabilities"] == null || totalAssetMap["Liabilities"] == "" ? 0 :(totalAssetMap["Liabilities"]/totallibility)*100) +"%</td>"+"</tr> ");

							$("#networthDetailsPopup").append("<tr class='grybg'><td><b> Total Liabilities</b></td>"+
									"<td><b>"+maskAmountValue(Math.round(totallibility))+"</b></td>"+
									"<td>100%</td>"+"</tr> ");
...
"</tr> ");*/
	...
	function onLoadAllPieCharts() {
		...
		//console.log("liabilitiesPieChartDataList" + JSON.stringify(liabilitiesPieChartDataList));
		Highcharts.chart('idassets', {
			chart: {
				plotBackgroundColor: null,
				plotBorderWidth: null,
