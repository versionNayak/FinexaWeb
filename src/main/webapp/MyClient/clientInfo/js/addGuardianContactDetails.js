...
$(document).ready(function() {
	...
	$("#idSave").on("click",function (event) {
		...
		if (v){
			...
			formData = $('#guardianContact').serializeToJSON();
 ...
 */ 			
			 var mode = sessionStorage.getItem("PAGE_MODE");
 			if (mode=="EDIT"){
 				//alert("inside guardian contact edit");
 				...
 				//console.log(data);
 			//	getClientDataWithErrorHandling("POST", data, "UpdateClientGuardianContactInfo", onSuccess,onError);
 				saveData("POST", data, "UpdateClientGuardianContactInfo", onSuccess);
 				...
 				/*function onError(err){
 				    $("#alertform").text("Error saving guardian contact information. Please try after sometime or contact system administrator.");
 					$(window).scrollTop(0);
 					hideLoaderOnSave("#idSave");
 				}*/
 			}
 			else{
 				var PAGE_MODE_ADDEDIT=sessionStorage.getItem("PAGE_MODE_ADDEDIT");
 				console.log("mode "+mode);
 				console.log("PAGE_MODE_ADDEDIT "+PAGE_MODE_ADDEDIT);
 				if(PAGE_MODE_ADDEDIT=="ADDEDIT"){
 					/*data = JSON.stringify(formData);
					var guardianInfo = JSON.parse(sessionStorage.getItem("GUARDIAN_PERSONAL_INFO"));
					var guardianInfo_json=JSON.stringify(guardianInfo);
					
					
					
					console.log("guardianInfo "+guardianInfo_json);
				
					
					getClientData("POST", guardianInfo_json, "ClientGuardianInfo", onSuccessGuardian);
	 				function onSuccessGuardian(data) {
	 					console.log("guardianID "+data.id);
	 					formData["guardianID"] = data.id;
	 					//data = JSON.stringify(formData);
	 					
	 					console.log("in guardian contact info "+data);
	 					getClientData("POST", data, "ClientGuardianContactInfo", onSuccess);
	 	 				function onSuccess(data) {
	 	 					console.log("add guardian success");
	 	 					sessionStorage.setItem("PAGE_MODE", "EDIT");
	 	 					editPage("clientInfo/addClient.html","Edit Client Personal Info ");
	 	 				}
	 				}*/
 					//console.log("formData "+formData);
 					//alert("inside ADDEDIT of guardian contact");
 					//console.log("sessionStorage.getItem "+sessionStorage.getItem("CLIENT_PERSONAL_INFO"));
					...
					clientPersonalInfo["clientGuardianContactDTO"] = formData;
					
					//console.log("before clientPersonalInfo = " + clientPersonalInfo);
					var clientInfo = JSON.stringify(clientPersonalInfo);
					//console.log("after clientPersonalInfo = " + clientInfo);
				
				//	getClientDataWithErrorHandling("POST", clientInfo, "clientMaster", onAddPersonalInfoSuccess,onAddPersonalInfoError);
					...
					/*function onAddPersonalInfoError(err){
	 				    $("#alertform").text("Error saving guardian contact information. Please try after sometime or contact system administrator.");
	 					$(window).scrollTop(0);
	 					hideLoaderOnSave("#idSave");
	 				}*/
 				...
 				//console.log(data);
 				sessionStorage.setItem("Guardian_CONTACT_INFO", data);
 				//console.log(data);
 				hideLoaderOnSave("#idSave");
...
function getSelectedGurdianContact(){
	var x=0;
	
	console.log("Adding contacts:Client ID : " + selectedClientId);
	var serviceUrl = "ClientGuardianContactInfo/guardian/" + selectedClientId;
	getClientData("GET", "", serviceUrl, onSuccess);
	function onSuccess(data) {
		console.log("Adding contacts: Email: " + data.emailID);
		contactID = data.id;
		if(contactID!=0){
		...
		$("#idEmergencyContact").val(data.emergencyContactNo);
		...
		console.log ("Office address: " + fullOfficeAddress);
		console.log ("Permanent Address: " + fullPermanentAddress);
		console.log ("Correspondence Address: " + fullCorrespondenceAddress);
		
		console.log ("Edit contacts 1");
		if (officeFlag != null && officeFlag != ''){ 	
			console.log ("Edit contacts 1-1");
			if (permanentFlag != null && permanentFlag != ''){
				console.log ("Edit contacts 1-2");
				if  (correspondenceFlag != null && correspondenceFlag != ''){
					console.log ("Edit contacts 1-3");
	...
	console.log("Adding contacts:23");
}
