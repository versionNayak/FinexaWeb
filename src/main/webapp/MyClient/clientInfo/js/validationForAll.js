...
//alert("inside validateDate");
	if (!(validDate.test(iDate))){
...
function checkValidDate(iDate) {
	...
	//alert("year: " + year);
	if (month > 12 || year > 2100 || year < 1000) {
		return false;
	} else {
...
//	console.log("In validate date"+pDate);
	this.pDate=pDate;
	var lDate=this.pDate;

    var eighteenYearsAgo = moment().subtract(18,'years');
   // console.log("eigh "+eighteenYearsAgo);
    var birthday = moment(lDate,'DD/MM/YYYY');
...
function isPastDate(iDate) {	
	...
	//return false;
    var date1 = moment(iDate,'DD/MM/YYYY');
    //alert('Input Date '+iDate);
...
/*function IsDateLessThanLifeExpectancy(startdate,bdateplusLifeExpectancy){
	alert("startdate "+startdate);
	alert("bdateplusLife "+bdateplusLifeExpectancy);
...
//	alert ("startdate: " + iDateReformatted);
	...
	//alert("startdate333: " + startdate333);
...
//	alert('Year'+ year);
	...
	alert("bdateplusLife333 "+bdateplusLife333);
	var bdateplusLife=new Date(bdateplusLife333.toString());
	alert("bdateplusLife "+bdateplusLife);
...
function IsYearLessThanLifeExpectancy(age, tenure, lifeExpectancy){
	
	var NumberOfYearsToStart=Number(tenure)+Number(age);
	
	if(NumberOfYearsToStart > lifeExpectancy){
		console.log("false");
		return false;
	}
	else {
...
function isCharForName(iText){
//	alert("Inside isChar");
	var chkAlpha = 0;
	this.iText = iText;
//	alert("Input: " + this.iText);
	if (!this.iText.match(charOnlyForName)){
//		alert("Not alpha");
		return false;
	}
	else{
...
function validatePan(iPanNo){
	...
	var panCode = /([C,P,H,F,A,T,B,L,J,G,E])/;
...
//alert ("Inside validate country code");
	...
	if (posPlus >0){
		...
		if (!testInputDataValue(lCountryCodeNums,numberOnly)){
...
function date1AfterDate2(date1, date2){
    ...
    var lDate1 = moment(date1Str, "DD/MM/YYYY");
    var lDate2 = moment(date2Str, "DD/MM/YYYY");
...
function isDateBetwenRange(ad_from,ad_to,ad_dt){
	if(ad_dt >= ad_from && ad_dt <= ad_to){
		return true;
	}else{
		return false;
	}
