...
//	return false;
	...
	lRecurringDepositAmount.style.border = "1px solid #ccc";
...
//	return false;
	...
	if (hasValue(lRecurringDepositAmount.value)){
		lRecurringDepositAmount.value = lRecurringDepositAmount.value.replace(/,/g, '');
		if (lRecurringDepositAmount.value == 0) {
			document.getElementById('alertDepositAmount').innerHTML="Value cannot be 0";
			lRecurringDepositAmount.style.border = "2px solid red";
			errSS=1;
		}
		else {
			var num = lRecurringDepositAmount.value;
			if (!isDecimal(num)) {
				...
				lRecurringDepositAmount.value = n;
			}
		}
	}
	else {
		if (!hasValue(lRecurringDepositAmount.value)) {
			document.getElementById('alertDepositAmount').innerHTML="Please enter Recurring Deposit Amount";
			lRecurringDepositAmount.style.border = "2px solid red";
			errSS=1;
		}
	}
	...
	var cfUser2 = lDepositFrequency.options[lDepositFrequency.selectedIndex].value;
	if (cfUser=="0"){
		document.getElementById('alertdf').innerHTML="Please enter Deposit Frequency";
